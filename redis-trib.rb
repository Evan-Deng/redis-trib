...
class RedisTrib
    ...
    def load_cluster_info_from_node(nodeaddr, auth=nil)
        node = ClusterNode.new(nodeaddr, auth)
        ...
        node.friends.each{|f|
            next if f[:flags].index("noaddr") ||
                    ...
                    f[:flags].index("fail")
            fnode = ClusterNode.new(f[:addr], auth)
            fnode.connect()
            next if !fnode.r
            begin
                fnode.load_info()
    ...
    def check_cluster_cmd(argv,opt)
        load_cluster_info_from_node(argv[0],opt['auth'])
        check_cluster
    end
    ...
    def create_cluster_cmd(argv,opt)
        opt = {'replicas' => 0, 'auth' => nil}.merge(opt)
        @replicas = opt['replicas'].to_i

        xputs ">>> Creating cluster"
        xputs opt.to_s
        argv[0..-1].each{|n|
            node = ClusterNode.new(n, opt['auth'])
            node.connect(:abort => true)
            node.assert_cluster
            node.load_info
            node.assert_empty
    ...
    def addnode_cluster_cmd(argv,opt)
        ...
        load_cluster_info_from_node(argv[1],opt["auth"])
        check_cluster
...
ALLOWED_OPTIONS={
    "create" => {"replicas" => true, "auth" => true},
    "add-node" => {"slave" => false, "master-id" => true, "auth" => true},
    "import" => {"from" => :required, "copy" => false, "replace" => false, "auth" => true},
    "reshard" => {"from" => true, "to" => true, "slots" => true, "yes" => false, "timeout" => true, "pipeline" => true, "auth" => true},
    "rebalance" => {"weight" => [], "auto-weights" => false, "use-empty-masters" => false, "timeout" => true, "simulate" => false, "pipeline" => true, "threshold" => true, "auth" => true},
    "fix" => {"timeout" => MigrateDefaultTimeout, "auth" => true},
    "check" => {"auth" => true},
    "info" => {"auth" => true}
}
